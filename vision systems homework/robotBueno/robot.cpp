...
//  COMPILACION General: 
// g++ -lboost_system -larmadillo -L/usr/local/include/player-3.0 -L/usr/local/include/Stage-4.3 -lplayerc++ -lstage robot.cpp -o robot
...
mat A(4,3);
mat B(4,4);
float s=0.0, theta=0.0,v=0.0, w=0.0, a=0.0, aw=0.0;
float pi=3.141593;


int N=10;
mat I1(N,1), I0(N,1), K(2,2), Ur(2,1), U0(N,1), U1(N,1);
...
//VERDADERO SANTO GRIAL!!!!! 
	...
	mat K(2,2), M(3,3);
	mat uref(2,1), uobservada(2,1), ut(2,1), ut1(2,1);
	mat iref(2,1), iobservada(2,1), it(2,1), it1(2,1);
	mat qref(3,1), qobservada(3,1), qt(3,1), qt1(3,1);
	mat tref(3,1), tobservada(3,1), tt(3,1), tt1(3,1);
//s=         ;
...
s=-0.00138468*pow(t,0)+0.0277875*pow(t,1)+-0.00695026*pow(t,2)+0.000772633*pow(t,3)+-4.2516e-05*pow(t,4)+1.13433e-06*pow(t,5)+-1.17092e-08*pow(t,6);
theta=-0.0101876*pow(t,0)+0.297666*pow(t,1)+-0.0836975*pow(t,2)+0.00854273*pow(t,3)+-0.000434988*pow(t,4)+1.13275e-05*pow(t,5)+-1.19281e-07*pow(t,6);

v=-0.00192671*pow(t,0)+0.0205783*pow(t,1)+-0.00366018*pow(t,2)+0.00027465*pow(t,3)+-9.15958e-06*pow(t,4)+1.12006e-07*pow(t,5);
w=0.297666*pow(t,0)+-0.167395*pow(t,1)+0.0256282*pow(t,2)+-0.00173995*pow(t,3)+5.66374e-05*pow(t,4)+-7.15683e-07*pow(t,5);

a=0.0205783*pow(t,0)+(-0.00366018*2)*pow(t,1)+(0.00027465*3)*pow(t,2)+(-9.15958e-06*4)*pow(t,3)+(1.12006e-07*5)*pow(t,4);
aw=-0.167395*pow(t,0)+(0.0256282*2)*pow(t,1)+(-0.00173995*3)*pow(t,2)+(5.66374e-05*4)*pow(t,3)+(-7.15683e-07*5)*pow(t,4);

	K<<0.02/2<<0.02/2<<endr
	 <<(2*0.02)/0.1;
...
while (E>0.01)
{
	...
	while( E>0.001)
	{
		...
		ut<<robot.GetXPos()<<endr
		  <<robot.GetYaw();
	

		it1=it+inv(K)*(uref-ut);
		ut1=ut+K*(it1-it);
		it=it1;
		E=uref(0,0)-ut(0,0);
		robot.SetSpeed(ut1(0,0)*4,ut1(0,1)*4);
	}
}


			robot.SetSpeed(v*4,w*4); 											//4,18,24,36,100
